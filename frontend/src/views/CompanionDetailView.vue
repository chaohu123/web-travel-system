...
import { api, companionApi, routesApi } from '../api'
...
async function fetchPost() {
  ...
  if (post.value?.teamId) {
    try {
      const res = await api.get<{ data: { data: { members: TeamMemberItem[] } } }>(`/companion/teams/${post.value.teamId}`)
...
async function quitCompanion() {
  ...
  try {
    ...
    // TODO: 与后端联调退出小队接口 /companion/teams/{teamId}/quit
    ElMessage.success('已模拟退出当前活动（示例前端逻辑）')
  } catch {
    // 用户取消
  }
}
...
// 初始化 ECharts 地图（示例：按照行程日期绘制一条时间轴路线折线）
function initMap() {
  ...
  plan.value.days.forEach((day) => {
    day.activities.forEach((act) => {
      xAxisLabels.push(`${day.dayIndex} 日 · ${act.name || '行程点'}`)
      seriesData.push(index * 10 + 5)
      index += 1
    })
  })
  ...
  chart.setOption({
    ...
    grid: { top: 40, left: 18, right: 16, bottom: 40 },
    tooltip: {
      trigger: 'axis',
    ...
    <template v-else-if="post">
      ...
      <section class="top-card">
        <div class="top-inner">
          <div class="top-main">
            <div class="user-row">
              <div class="avatar-wrap">
                <img v-if="post.creatorAvatar" :src="post.creatorAvatar" :alt="post.creatorNickname" class="avatar-img" />
            ...
            <div class="trip-brief">
              ...
              <div class="meta-row">
                ...
                <div class="style-meta">
                  ...
                  <div class="tag-row">
                    <ElTag size="small" type="success" effect="plain">文化</ElTag>
                    <ElTag size="small" type="info" effect="plain">美食</ElTag>
                    <ElTag size="small" type="warning" effect="plain">自然</ElTag>
                  </div>
                </div>
          ...
          <div class="top-actions">
            ...
            <ElButton size="large" class="btn-outline" @click="quitCompanion">
              退出活动
            </ElButton>
                  ...
                  <ElCollapse v-model="activeAccordion" class="trip-accordion">
                    ...
                    >
                      ...
                      <div class="day-content">
                        ...
                        >
                          <div class="act-image">
                            <div class="act-image-placeholder" />
                          </div>
                          <div class="act-body">
              ...
              <ElTabPane label="地图路线" name="map">
                <section class="map-section">
                  ...
                  <div class="map-card">
                    ...
                    <p class="map-tip">
                      当前示例使用 ECharts 将每日行程抽象为一条时间轴折线。接入高德 / 百度地图后，可将每个景点映射为真实地图坐标。
                    </p>
                  </div>
                </section>
              </ElTabPane>
            ...
            <div v-if="teamMembers.length" class="members-card">
              ...
              >
                ...
                <div class="member-info">
                  <span class="member-name">{{ m.userName || '旅友' }}</span>
      ...
      >
        <div v-if="selectedMember" class="drawer-member">
          ...
          <h3 class="drawer-name">{{ selectedMember.userName || '旅友' }}</h3>
          <p class="drawer-role">{{ selectedMember.role === 'leader' ? '队长' : '成员' }}</p>
          <p class="drawer-state text-subtle">状态：{{ selectedMember.state }}</p>
        </div>
      </ElDrawer>
    </template>
