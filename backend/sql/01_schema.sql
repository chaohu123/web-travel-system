...
-- 旅游结伴/行程系统 - 完整数据库建表脚本
-- ============================================
-- 包含：用户、行程、结伴、游记、动态、评论、消息、结伴帖聊天等全部表结构
-- 建议先创建数据库后执行本脚本
-- ============================================
...
DROP TABLE IF EXISTS `t_post_chat_message`;
CREATE TABLE `t_post_chat_message` (
  `id`         BIGINT       NOT NULL AUTO_INCREMENT COMMENT '消息ID',
  `post_id`    BIGINT       NOT NULL COMMENT '结伴帖ID',
  `user_id`    BIGINT       NULL COMMENT '发送者用户ID，NULL 表示系统消息',
  `content`    VARCHAR(2000) NOT NULL COMMENT '消息内容',
  `type`       VARCHAR(16)  NULL DEFAULT 'text' COMMENT '消息类型: text=文本，spot=景点卡片',
  `spot_json`  VARCHAR(1000) NULL COMMENT '景点卡片时的 JSON 数据',
  `created_at` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '发送时间',
  PRIMARY KEY (`id`),
  KEY `idx_post_chat_post_created` (`post_id`, `created_at`),
  CONSTRAINT `fk_post_chat_post` FOREIGN KEY (`post_id`) REFERENCES `t_companion_post` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_post_chat_user` FOREIGN KEY (`user_id`) REFERENCES `t_user` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='结伴帖内置沟通消息';
...
CREATE TABLE `t_interaction_message` (
  ...
  `created_at`      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_msg_recipient_read` (`recipient_id`,`is_read`,`created_at`),
...
CREATE TABLE `t_private_conversation` (
  ...
  `user1_deleted_at`     DATETIME     NULL COMMENT '用户1删除会话时间',
  `user2_deleted_at`     DATETIME     NULL COMMENT '用户2删除会话时间',
  `created_at`           DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at`           DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_private_message` (
  ...
  `content`         MEDIUMTEXT  NOT NULL COMMENT '消息内容（支持图片 Base64 及长文本）',
  `type`            VARCHAR(16)      DEFAULT 'text' COMMENT '消息类型: text/image/route/companion/spot',
  `spot_json`       TEXT        NULL COMMENT 'type=spot 时景点数据 JSON',
  `created_at`      DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
